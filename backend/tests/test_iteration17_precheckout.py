...
# --- RevOrder per-domain integration ---

DOMAIN = "purepeptide.bg"


class TestRevOrder:
    def test_get_settings_admin_required(self):
        r = requests.get(f"{API}/admin/integrations/revorder", timeout=10)
        assert r.status_code in (401, 403)

    def test_full_lifecycle(self, admin_session):
        # get current
        r = admin_session.get(f"{API}/admin/integrations/revorder", timeout=10)
        assert r.status_code == 200
        data = r.json()
        assert "domains" in data and "events" in data

        # save
        r = admin_session.put(f"{API}/admin/integrations/revorder", json={
            "domain": DOMAIN,
            "api_key": "test-key",
        ...
        r = admin_session.get(f"{API}/admin/integrations/revorder", timeout=10)
        assert r.status_code == 200
        cfg = r.json()["domains"].get(DOMAIN)
        assert cfg is not None
        assert cfg["api_key"] != "test-key"  # masked
        assert "•" in cfg["api_key"]
        assert "•" in cfg["secret_key"]
        assert cfg["enabled"] is True

        # test endpoint
        r = admin_session.post(f"{API}/admin/integrations/revorder/test",
                               json={"domain": DOMAIN}, timeout=15)
        assert r.status_code == 200, r.text[:300]
        result = r.json()
        assert "sent" in result

        # Get an existing order id to reference
        r = admin_session.get(f"{API}/admin/orders", params={"limit": 1}, timeout=10)
        assert r.status_code == 200
        orders = r.json().get("orders") or []
        assert orders, "no orders to reference for webhook"
        ext_id = orders[0]["id"]

        # webhook wrong sig -> 401
        body = json.dumps({"id": f"evt_iter17_{int(time.time())}", "external_id": ext_id,
                           "tracking_number": "TRK123", "status": "shipped"},
                          separators=(",", ":")).encode()
        r = requests.post(f"{API}/webhooks/revorder/{DOMAIN}",
                          data=body,
        ...
        # Verify order got tracking_number update
        r = admin_session.get(f"{API}/admin/orders/{ext_id}", timeout=10)
        if r.status_code == 200:
            o = r.json()
            if "order" in o:
                o = o["order"]
            tracking = o.get("tracking") or {}
            assert tracking.get("tracking_number") == "TRK123" or \
                o.get("fulfillment_status") in ("shipped", "fulfilled"), \
                f"order not updated: tracking={tracking}, fulfillment={o.get('fulfillment_status')}"
