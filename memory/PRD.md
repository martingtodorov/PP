...
- Колекции: H1 беше винаги скрит (`pp-seo-h1`), защото се предполагаше заглавие в описанието — но
  нито едно описание от Shopify няма заглавие → страниците бяха без заглавие. Сега H1 се вижда, ако
  описанието няма `<h1-3>`; за „Всички“ се показва „Всички пептиди“ (`t("catalog")`), а импортът
  записва това заглавие за ALL_HANDLE вместо Shopify-ското „Всички“.

### 2026-06 — продуктова страница като purepeptide.bg + „Преведи абсолютно всичко“
- Продукти: заглавието от Shopify тялото („Какво е Ретатрутид?“) е **H1**, името на продукта е **H2**
  (както на живия сайт). Импортът го пази (`clean_body(keep_h1=True)` само за продукти); за вече
  импортирани продукти `restore_headings.py` го връща при всеки старт от последния Matrixify експорт
  (или bundled `data/matrixify-export.xlsx`), без да пипа снимки/цени/преводи — 22/23 продукта в preview.
- Layout (desktop): галерията е sticky вляво (`lg:sticky lg:top-24`), описанието е в дясната колона
  под панела за покупка; на мобилно описанието идва след акордеоните.
- Нова админ страница **/admin/translations** („Преводи“): един бутон „Преведи всичко на N езика“ →
  продукти, колекции, статии, страници, настройки (лента/слоган/футър → `*_i18n`), чекаут/UI текстове;
  избор на езици, „Презапиши съществуващите“, прогрес, текущ елемент, неуспешни, „Спри“, история.
- Опашката е durable: `translate_jobs.completed[]` пази свършеното; при рестарт/deploy
  `resume_translate_jobs()` продължава от там (проверено: 5/64 → продължи). `POST /admin/translate/bulk/stop`.
- Layout ползва `settings.tagline_i18n[locale]` / `footer_text_i18n[locale]`, когда има превод.
- Тестове: `tests/test_product_headings.py`; реален превод „всичко“ за RO пуснат в preview.
- Колекции: Shopify тялото на колекциите също носи `<h1>` („Пептиди, изследвани за отслабване и
  метаболизъм“, „Всички пептиди“) — импортът го пази (`keep_h1=True`), `restore_headings.py` го
  върна на 8/8 колекции; страницата не рендерира отделно заглавие, когато описанието има h1.
- `i18n.ai_translate` пропускаше полета извън `TRANSLATABLE` → настройките (слоган/лента/футър) не
  се записваха; вече допуска ключовете на подадения източник. `_translate_one` третира превод като
  остарял, ако българският текст има `<h1>`, а преводът — не (без нужда от „презапиши“).
- Реален „Преведи всичко“ за RO: 64/64, 0 неуспешни (~30 мин; статиите са най-бавни). iteration_33: всичко ✅.

### 2026-06 — RevOrder: „Тествай връзката“ даваше 405
- Причина: интеграцията е била построена на предположение — POST към
  `https://api.nextcartmanager.com/api/orders`, а RevOrder там приема само GET (Allow: GET, HEAD).
  Документацията им (`/api/docs`) е само след логин. Скриптът, който RevOrder дава на клиента
  (`client.nextcartmanager.com/checkout/script/<id>.js`), в момента е празен: `platform=custom:absent`
  — т.е. RevOrder чака клиентът да въведе при тях API адрес/ключ на нашия магазин („custom platform“).
- Тестът вече проверява това, което е наше: подписан POST към публичния ни webhook URL (200 + валиден
...
### 2026-06 — „Нищо не се превежда“ на purepeptide.gr (истинската причина)
**Диагноза**: преводите (за 40$ Claude) СЪЩЕСТВУВАТ в продукцията — проверено през живото API:
продукти, колекции, `menu_title`, страници и статии са преведени за всичките 10 не-BG езика.
Проблемът беше, че `lib/api.js currentLocale()` (както и `LocaleContext`) определяше езика **само по
префикса в URL**, а purepeptide.gr обслужва гръцки от корена → всяка заявка към API-то искаше `locale=bg`.
Затова UI-ят беше гръцки (статични низове), но всички имена, заглавия, описания и цените (лв.) бяха български.
**Поправено**:
- `currentLocale()` и `LocaleContext` вече падат на `localeFromHost()` — езикът на домейна.
- Проверено: домейн, насочен към preview → цялият сайт (начало, каталог, продукт, FAQ, карта на сайта,
  количка, чекаут) излиза на румънски/гръцки, с локална валута.
**Локализирани и останалите твърдо зашити български надписи** (42 нови ключа × 11 езика в
`checkoutStrings.js` + `ui_strings.SOURCE_BG`): breadcrumbs (Начало/Всички пептиди/Научни статии),
цялата страница „Моят профил“ (заглавия, полета, бутони, статуси на поръчките, брой артикули),
картата на сайта, footer линкът, валидиращите грешки в чекаута („Моля, попълнете: …“ + имената на
полетата), JSON-LD описанието, skip-link-ът в `index.html` (има си и inline скрипт за домейните).
Допълнени и 6-те липсващи `locate*` низа за de/cz/hu/pl/sk/si/gr/ro и 6 за fr (стар пропуск —
`tests/test_ui_strings.py` вече минава).
**Имейлите** също: отказана поръчка, получено плащане и „изпратена пратка“ вече минават през
локализираните шаблони (`render_cancelled`, `render_payment_received`, `render_shipment`) — 11 езика.
**Забележка за следващия агент**: редът на езиковите блокове в `checkoutStrings.js` е
`bg, en, ro, pl, cz, hu, de, fr, sk, si, gr` — НЕ по азбучен ред. Първата вкарване на ключове ползва
грешен ред и подреди преводите разбъркано (iteration_40 го хвана); поправено с
`backend/tools/realign_ui_keys.py`. Винаги вкарвай ключове по име на блока, не по позиция.
**Тестове**: iteration_40 (multi-locale audit) → след поправката /gr, /ro, /pl, /de, /si, /sk, /fr
показват правилните надписи; `tests/test_ui_strings.py` 7/7.
**Известен дълг**: 67 стари e2e теста в `backend/tests/` падат, защото са писани срещу предишните
preview данни (23 vs 21 продукта, стари handles преди ротацията, стари промо кодове) — не са регресии
от тази сесия, но чакат обновяване.

### 2026-06 — Премахнати фирмени данни от правните страници
Собственикът поиска (изрично, вариант „махни всичко, включително PurePeptide“):
- `backend/tools/strip_company_details.py` (`--dry` за преглед) премахва от `pages`:
  ЕИК/CUI `208640029`, седалището (гр. София, ул. Бяла река №12 бл. 10 ап. 3), клаузата
  „собственост и управляван от …“ и заменя всяко споменаване на фирмата с локализирано
  „Търговецът/Търговеца“ (Comerciantul, the Merchant, ο Έμπορος/τον Έμπορο, der Händler/dem Händler,
  Sprzedawca, Obchodník, Trgovec, A Kereskedő, le Vendeur) с правилен падеж след предлог,
  включително гръцкия член. Изчиства и осиротелите абзаци, останали само с името.
- Страниците се пазят по един документ на език (`pages.locale`), скриптът минава през всички.
- Засегнати в preview: `terms-conditions` (bg, ro), `terms-of-service` (bg), `delivery-and-payment` (bg, ro),
  `shipping-policy` (bg). Проверено: няма остатъци, изреченията са четими, домейнът www.purepeptide.bg остава.
- **SEO полетата** (`seo_title`, `seo_description`) запазват марката „PurePeptide“, но без „ЕООД“ —
  иначе Google резултатите щяха да гласят „Общи условия | Търговецът“.
- Данните на подателя за куриера (NextLevel sender) НЕ са пипани — те са нужни за товарителниците.
...
## 2026-06-07 — фирмата не се споменава в имейлите
...
## 08.06.2026 — актуален обхват (добавено в тази сесия)
- Анализи: локален (софийски) ден, 24-часова ос, гео по държави/градове, live 5 мин. и 24ч по
  локация, карта на Европа, топ страници, източници, дневен отчет по имейл.
- Каталог: вътрешни (невидими за клиента) тагове по продукти; delist на колекции.
- Админ: 90-дневна сесия, инвалидирана при смяна на паролата.
- Статии: без автори никъде (публичен API, JSON-LD, импорт).
- .eu/en винаги английски; HEAD 200 на всички HTML страници.
- Остава: продуктови ревюта (P1), карта/отчет — потвърждение на реалното изпращане на продукшън.

- 08.06.2026 (продължение): 301 препратките са отделна функция (`/admin/redirects`), ротацията
  остава твърд 404. Подредбата на продуктите е защитена от пресийдване (`ALLOW_RESEED`) и се
  възстановява при старт от `product_orders`.

## Решения от 08.06.2026
- **Плащания по пазари**: България — САМО наложен платеж (банковият превод е изключен по решение на
  собственика). ES/FR/BE/NL/CY — само предплатено (банков превод). DE и останалите — и двете.
  Източник на истината: `COUNTRY_PAYMENTS` в `backend/nextcart.py`; сървърът налага правилото при
  чекаут чрез `payment_method_for()`.
- **Деплоят е неутрален към съдържанието**: нито едно динамично поле (наличности, описания, линкове,
  подредба, админ тагове, изтеглени линкове, 301 препратки, sitemap) не се променя от деплой или
  рестарт. Каталожен импорт се пуска САМО ръчно с `-e run_catalog_import=true`, и дори тогава пази
  наличностите на живо и админ полетата. Еднократните миграции минават през `run_once()`.
- **Анализи**: таблото се прекалкулира на всеки 5 минути във фонов процес (`ANALYTICS_WARM_SEC`),
  админът чете кеш; Live броячите (5 мин.) са винаги на момента.
